...
SOURCE_SCRIPT_DETECTORS = {
    "ja": ("kana", re.compile(r"[\u3041-\u309f\u30a0-\u30ff\uff66-\uff9f]")),
    "zh": ("han", re.compile(r"[\u3400-\u9fff\uf900-\ufaff]")),
    "ko": ("hangul", re.compile(r"[\u1100-\u11ff\u3130-\u318f\uac00-\ud7af]")),
    "ru": ("cyrillic", re.compile(r"[\u0400-\u052f]")),
    "uk": ("cyrillic", re.compile(r"[\u0400-\u052f]")),
    "bg": ("cyrillic", re.compile(r"[\u0400-\u052f]")),
    "el": ("greek", re.compile(r"[\u0370-\u03ff\u1f00-\u1fff]")),
    "ar": ("arabic", re.compile(r"[\u0600-\u06ff\u0750-\u077f\u08a0-\u08ff]")),
    "fa": ("arabic", re.compile(r"[\u0600-\u06ff\u0750-\u077f\u08a0-\u08ff]")),
    "ur": ("arabic", re.compile(r"[\u0600-\u06ff\u0750-\u077f\u08a0-\u08ff]")),
...
def locale_language(locale: str | None) -> str:
    return (locale or "").split("-", 1)[0].casefold()


def discover_project_json(source_path: Path) -> Path | None:
...
def read_project_locales(path: Path | None) -> tuple[str, str]:
    ...
    if not isinstance(value, dict):
        return "", ""
    return str(value.get("source_locale") or ""), str(value.get("target_locale") or "")


def locale_row_applies(row: dict[str, str], source_locale: str, target_locale: str) -> bool:
    row_source = (row.get("source_locale") or "").strip().casefold()
    row_target = (row.get("target_locale") or "").strip().casefold()
    return (not row_source or row_source == source_locale.casefold()) and (
        not row_target or row_target == target_locale.casefold()
    )
...
def term_matches(text: str, term: str, row: dict[str, str]) -> bool:
    if not term:
        return False
    case_sensitive = parse_bool(row.get("case_sensitive") or "")
    flags = 0 if case_sensitive else re.IGNORECASE
    mode = (row.get("match_mode") or "substring").strip().casefold()
    if mode == "regex":
        try:
...
def main() -> int:
    ...
    parser.add_argument("--min-length-ratio", type=float)
    ...
    project_json = (
        ...
        else discover_project_json(source_path)
    )
    project_source_locale, project_target_locale = read_project_locales(project_json)
    source_locale = args.source_locale or project_source_locale
    target_locale = args.target_locale or project_target_locale
    source_language = locale_language(source_locale)
    target_language = locale_language(target_locale)
    ratio_min, ratio_max = PAIR_LENGTH_RATIOS.get(
        (source_language, target_language), DEFAULT_LENGTH_RATIO
    )
    if args.min_length_ratio is not None:
        ratio_min = args.min_length_ratio
    if args.max_length_ratio is not None:
        ratio_max = args.max_length_ratio
    if ratio_min < 0 or ratio_max <= 0 or ratio_min >= ratio_max:
        parser.error("length-ratio bounds must satisfy 0 <= min < max")
    detector = SOURCE_SCRIPT_DETECTORS.get(source_language)
    if detector and detector[0] in LANGUAGE_SCRIPTS.get(target_language, set()):
        detector = None
    report_path = (
        args.report.resolve()
        if args.report
    ...
    for entry_id in expected_order:
        ...
        if translation != unicodedata.normalize("NFC", translation):
            add_issue(
                ...
                "translation is not NFC-normalized",
                entry_id,
            )
        ...
        residual = detector[1].findall(translation_visible) if detector else []
        if residual:
            add_issue(
                issues,
                "warning",
                "residual-source-script",
                f"contains residual {detector[0]} characters, sample: {''.join(residual[:12])}",
                entry_id,
            )
        if source_visible:
        ...
        applicable_glossary = [
            row
            for row in glossary
            if scope_applies(row, source)
            and locale_row_applies(row, source_locale, target_locale)
        ]
        for row in applicable_glossary:
            term_source = (row.get("source") or "").strip()
            term_target = (row.get("target") or "").strip()
            status = (row.get("status") or "").strip().lower()
            if status == "forbidden" and term_matches(translation, term_target, row):
                add_issue(
                    issues,
                    "error",
                    "forbidden-term",
            ...
            if (
                term_source
                and term_matches(source_text, term_source, row)
                and term_target
                and not glossary_row_is_shadowed(
                    row, applicable_glossary, source_text, translation
                )
            ):
                if status == "locked" and not term_matches(translation, term_target, row):
                    add_issue(
                        issues,
                        "error",
                        "locked-term",
                ...
                elif status == "preferred" and not term_matches(translation, term_target, row):
                    add_issue(
                        issues,
                        "warning",
                        "preferred-term",
    ...
    report = {
        ...
        "length_ratio_bounds": [ratio_min, ratio_max],
        "errors": errors,
        "warnings": warnings,
        "issues": issues,
    }
